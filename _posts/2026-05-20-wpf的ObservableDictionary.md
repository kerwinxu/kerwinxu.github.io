---
layout: post
title: "wpf的ObservableDictionary"
date: "2026-05-20"
categories: ["计算机语言", "csharp"]
---

mvvmlight中有ObservableCollection，是动态数据集合类，只能放列表，ObservableDictionary是动态数据字典类，可以放键名键值的。
...
可以指定键名和键值的类型，初始化如下
```c#
MAreaBools = new ObservableDictionary<string, bool>();
```
在xaml中可以这样读取，不用加字符串
```xml
                    <Button Grid.Column="9" Content="清零" Command="{Binding TooglePlcMemoryBool}" CommandParameter="M0.2" Background="{Binding MAreaBools[M0.2], Converter={StaticResource BoolToBrushConverter}}" />
