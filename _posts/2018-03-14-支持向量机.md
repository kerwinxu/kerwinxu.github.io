...
- 求 ：尝试找到一个超平面$ \\theta\_0+\\theta\_1x\_1+\\cdots+\\theta\_nx\_n=0$，能够把二元数据隔开.
...
在分离超平面固定为 $ w^Tx+b=0$的时候，$ |w^Tx+b|$表示点到超平面的距离（注意实际距离应该还有个分母，为L2范式的），通过观察 $ |w^Tx+b|$与$ y$是否同号，我们判断分类是否正确，这里我们引入函数间隔概念 ，定义函数间隔 $ \\gamma'$

$$\\gamma'=y(w^Tx+b)$$

可以看到，他就是感知机模型里误分类到超平面距离的分子，对应训练集m个样本点对应的m个函数间隔的最小值，就是整个训练集的函数间隔。

在感知机模型中我们也提到，当分子成比例增大时，分母也成比例增大，为了统一度量，我们加上约束条件，这样我们就得到几何间隔$ \\gamma$，
...
如下图，分离超平面为$ w^Tx+b=0$,如果所有的样本不光可以被超平面分开，还和超平面保持一定的函数距离（下图函数距离为1），那么这样的超平面是比感知器模型优的，可以证明，这样的超平面只有一个，和超平面平行的保持一定函数距离的这两个超平面对应的向量，我们定义为支持向量，如下图虚线表示。

[![no img]](http://127.0.0.1/wp-content/uploads/2018/03/1042406-20161124144326487-1331861308.jpg)

支持向量到超平面的距离为$ \\frac{1}{||w||\_2}$,
...
一般我们都取函数间隔$ \\gamma'$为1，那我们的优化函数可以定义为

$$ max\\ \\  \\gamma =\\frac{1}{||w||\_2}  {\\color{red}{s.t }}y\_i(w^Tx\_i+b)=\\gamma^{'(i)}\\ge 1(i=1,2,\\cdots,m)$$

也就是说我们要在约束条件$ y\_i(w^Tx\_i+b)=\\gamma^{'(i)}\\ge\\gamma'(i=1,2,\\cdots,m)$下，最大化 $ \\frac{1}{||w||\_2|}$，可以看出，感知机模型是固定分母优化分子，而SVM是固定分子优化分母，同时加上支持向量限制。

由于$ \\frac{1}{||w||\_2|}$最大化等于 $ \\frac{1}{2}||w||\_2^2$的最小化(前面分母最小的时候是最大值，后边是分子最小的时候是最小值)，这样SVM模型等价于

$$ min\\ \\  \\frac{1}{2}||w||\_2^2 {\\color{red}{s.t }}y\_i(w^Tx\_i+b)=\\gamma^{'(i)}\\ge 1(i=1,2,\\cdots,m)$$

由于目标函数 $ \\frac{1}{2}||w||\_2^2$是凸函数，同时约束条件不等式是仿射的，根据凸优化理论，我们可以通过[拉格朗日函数](http://127.0.0.1/?p=1437)将我们的优化目标转化成无约束的优化函数，
...
先求w和b的最小值即 $ \\underbrace{min}\\limits\_{w,b}L(w,b,a)$，这个值可以通过分别对w和b求偏导得到。

L2范数就是$ ||A||2=\\sqrt{\\sum\\limits\_{ij}|A\_{ij}|^2}=\\sqrt{Tr(AA^T)}$，而这个对A的偏导就是2A

求导

$ \\frac{\\partial L}{\\partial w}=0\\Rightarrow w=\\sum\\limits\_{i=1}^{m}a\_iy\_ix\_i$

$ \\frac{\\partial L}{\\partial b}=0\\Rightarrow \\sum\\limits\_{i=1}^{m}a\_iy\_i=0$

从上两式可以看出，我们已经求得w和a的关系，只要我们后边接着能够求出优化函数极大化对应的a，就可以求出我们的w了，至于b，由于上式中没有b，所以我们最后的b可以有多个。

好了，既然我们已经求出w和a的关系，就可以代入优化函数$ L(w,b,a)消去w$了。，我们定义
...
从上边可以看出，通过对w和b极小化以后，我们的优化函数 $ \\psi(a)$中仅仅有a向量做参数，只要我们能够极大化这个，就可以求出此时对应的a，进而求出w和b
...
只要我们求出上式极小化时对应的a，就可以求w和b了，得用SMO算法来求，这里假设我们已经求出a的值$ a^\*$，那么

$$ w^\*=\\sum\\limits\_{i=1}^{m}a\_i^\*y\_ix\_i$$

求b ，对于任意向量$ (x\_x,y\_s)$,都有

$$y\_s(w^Tx\_s+b)=y\_s(\\sum\\limits\_{i=1}^{m}a\_iy\_ix\_i^Tx\_s+b)=1$$

假设我们有s个支持向量，最对应我们可以求出s个$ b^\*$，理论上这些都可以作为最终的结果，但是我们一般采用更健壮的方法，即求出所有支持向量所对应的 $ b\_s^\*$,然后将平均值作为最后的结果。

怎么得到支持向量呢？根据KKT条件中的对偶互补条件 $ a\_i^\*(y\_i(w^Tx\_i+b)-1)=0$,如果$ a\_i>0$则有 $ (y\_i(w^Tx\_i+b)-1)=0$,即点在支持向量上，否则$ a\_i=0$,则有则有 $ y\_i(w^Tx\_i+b)\\ge 1 $,即样本在支持向量上或者被正确分类。

## 线性可分SVM的算法过程

- 输入数据
    - 线性可分的m个样本，$ (x\_1,y\_1),(x\_2,y\_2),\\cdots,(x\_m,y\_m)$,
        - 其中x为n维特征向量，
        - y为二元输出，值为1或者-1
- 输出
    - 分类超平面的函数参数 $ w^\*和b^\*$和分类决策函数
- 算法过程
    ...
    2. 用SMO算法求出上式最小化时对应的a向量值 $ a^\*$向量。
    3. 计算$ w^\*=\\sum\\limits\_{i=1}^{m}a\_i^\*y\_ix\_i$
    4. 找出所有的s个支持向量，即满足$ a\_s>0$对应的样本$ (x\_s,y\_s)$,通过$ y\_s(\\sum\\limits\_{i=1}^{m}a\_iy\_ix\_i^Tx\_s+b)=1$，计算每个支持向量$ (x\_s,y\_s)$对应的$ b\_s^\*$,，计算出 $ b\_s^\*=y\_s-\\sum\\limits\_{i=1}^{m}a\_iy\_ix\_i^Tx\_s$，所有的$ b\_s^\*$对应的平均值即是最终的 $ b^\*=\\frac{1}{s}\\sum\\limits\_{i=1}^{s}b\_s^\*$
    5. 最终的分类超平面 $ w^\*\\cdot x+b^\*=0$，最终的分类决策函数为 $ f(x)=sign(w^\*\\cdot x+b^\*)$
...
[![no img]](http://127.0.0.1/wp-content/uploads/2018/03/1042406-20161125104106409-1177897648.png)

另外一种没有糟糕到不可分，但是会严重影响模型的泛化效果，如下图，本来可以是红色的线表示的，但因为有个蓝色的异常点，导致成虚线的了。

[![no img]](http://127.0.0.1/wp-content/uploads/2018/03/1042406-20161125104737206-364720074.png)
...
SVM对训练集里面的每个样本 $ (x\_i,y\_i)$引入一个松弛变量，$ \\xi\_i\\ge 0$,使得函数间隔加上松弛变量大于等于1，也就是

$$y\_i(w\\cdot x+b)+\\xi\_i \\ge 1，也就是y\_i(w\\cdot x+b)\\ge 1-\\xi\_i$$

对比硬间隔最大化，软间隔的要求放松了很多，之前一定要大于等于1，现在只需要加上一个大于等于0的松弛变量能大于等于1就可以了，当然，松弛变量不能白加，这是有成本的，每个松弛变量$ \\xi\_i$，对应一个代价$ \\xi\_i$,这样就得到了软间隔最大化SVM学习条件如下
...
- 也就是说，我们希望 $ \\frac{1}{2}||w||\_2^2$尽量小，误分类的点尽量的少，C是协调两者关系的正则化惩罚参数，在实际应用中，需要调参来选择。
...
首先我们来求优化函数对于$ w,b,\\xi$的极少值，这个可以通过求偏导得到
...
对于后边3个公式，我们可以消去$ \\mu\_i$(当这个为0)，只保留$ a\_i$，也就是说$ 0 \\leq \\alpha\_i \\leq C$,同时将优化函数目标变负号，求极少值。
...
和硬间隔的最大化线性可分SVM相比，我们仅仅是多了一个约束条件$ 0 \\leq \\alpha\_i \\leq C$

## 软间隔最大化时的支持向量

硬间隔最大化时求支持向量比较简单，

- 就是满足$ y\_i(w^Tx\_i + b) -1 =0$就可以了
    - $ \\alpha\_{i}^{\*}(y\_i(w^Tx\_i + b) - 1) = 0$对偶互补条件
        - 如果有 $ a\_i^\*>0 则有y\_i(w^Tx\_i+b)=1$,即点在支持向量上。
        - 如果有 $ a\_i^\*=0 则有y\_i(w^Tx\_i+b)\\ge 1$,即样本在支持向量上或者已经被正确分类

软间隔最大化，因为引入了一个松弛变量，我们从下图来研究软间隔最大化时支持向量的情况。

[![no img]](http://127.0.0.1/wp-content/uploads/2018/03/1042406-20161125133202346-307657619.jpg)

- 第i个点对应类别支持向量的距离为$ \\frac{\\xi\_i}{||w||\_2}$,根据软间隔最大化时KKT条件中的对偶互补条件 $ \\alpha\_{i}^{\*}(y\_i(w^Tx\_i + b) - 1 + \\xi\_i^{\*}) = 0$,我们有
    - 如果a=0，那么$ y\_i(w^Tx+b)-1\\ge 0$,即样本在支持向量上或者已经被正确分类。
    - 如果 $ 0 < a < C ,那么 \\xi\_i=0 ,y\_i(w^Tx+b)-1=0$，即点在支持向量上，如图中虚线支持向量上的点。
    - 如果a=C，说明这个是一个比较异常的点，需要检查 $ \\xi\_i$.
        - 如果 $ 0\\le \\xi\_i <1$,说明点被正确分类，但却在超平面和自己类别的支持向量中间，如图中的2和4
        - 如果 $ \\xi\_i=1$,那么这个点在超平面上。
        - 如果 $ \\xi\_i>1$，那么点在超平面的另一侧，也就是说，这个点不能被正确分类，如图中的1和3.
...
- 输入：
    - 线性可分的样本$ (x\_1,y\_1),(x\_2,y\_2),\\cdots,(x\_m,y\_m)$
        - x为n维特征向量。
        - y为一元输出，值为1或者-1
- 输出：
    - 分类超平面的参数$ w^\*和b^\*$
...
- 思考过程：
    - 因为存在异常点，所以这里加上一个松弛变量，之前的函数间隔是一定要大于等于1，而现在只要加上这个松弛变量大于等于1就可以了。 $$y\_i(w\\bullet x\_i +b) \\geq 1- \\xi\_i$$
        - 这里$ w,b,\\xi$都要尽量少，最好只是等于1就可以了，但这个等于1的，在硬间隔中，就表示支持向量。
    ...
    - 我们可以先求优化函数对$ w,b,\\xi$的最小值，然后再求拉格朗日系数$ a,\\mu$的最大值。
        - 求$ w,b,\\xi$最小值，
        ...
        - 优化目标完整公式1 \\begin{align}\\underbrace{ max }\_{\\alpha} \\sum\\limits\_{i=1}^{m}\\alpha\_i - \\frac{1}{2}\\sum\\limits\_{i=1,j=1}^{m}\\alpha\_i\\alpha\_jy\_iy\_jx\_i^Tx\_j\\\\s.t. \\; \\sum\\limits\_{i=1}^{m}\\alpha\_iy\_i = 0\\\\C- \\alpha\_i - \\mu\_i = 0\\\\\\alpha\_i \\geq 0 \\;(i =1,2,...,m)\\\\\\mu\_i \\geq 0 \\;(i =1,2,...,m)\\end{align}
            - 这里的a是要求最大值，所以有 $ \\underbrace{max}\_a$
            - 对于后边3个公式，因为第一个公式只有 a,y,x，所以我们可以消去 $ \\mu\_i$，也就是 $ 0 \\leq \\alpha\_i \\leq C$
...
- 算法过程如下：
    ...
    - 用SMO算法求出上式最小时对应的a向量的值 $ \\alpha^\*$向量。
    - 计算 $ w^{\*} = \\sum\\limits\_{i=1}^{m}\\alpha\_i^{\*}y\_ix\_i$
    - 找出所有的S个支持向量，即满足$ 0 < \\alpha\_s < C$对应的样本$ (x\_s,y\_s)$,通过 $ y\_s(\\sum\\limits\_{i=1}^{S}\\alpha\_iy\_ix\_i^Tx\_s+b) = 1$计算出每个支持向量$ (x\_s,y\_s)$对应的 $ b^\*$,用 $ b\_s^{\*} = y\_s - \\sum\\limits\_{i=1}^{S}\\alpha\_iy\_ix\_i^Tx\_s$，所有的平均值即为最终的 $ b^{\*} = \\frac{1}{S}\\sum\\limits\_{i=1}^{S}b\_s^{\*}$
    - 最终结果
...
我们令 $ x\_0 = 1, x\_1 = x\_1, x\_2 = x\_2, x\_3 =x\_1^{2}, x\_4 = x\_2^{2}, x\_5 = x\_{1}x\_2$，就得到下式
...
假设$ \\phi$是一个低维输入空间$ \\chi$(欧式空间的子集或者离散集合），到高维的希尔伯特空间的$ \\mathcal{H}$，那么如果存在函数 $ K(x,z)$,对于任意的 $ x,z \\in \\chi$，都有

$$K(x,z)=\\chi(x)\\cdot\\chi(z)$$

则称$ K(x,z)$为核函数，$ \\chi(x)$为映射函数，比如原始函数的内积为 <x,z>，映射后为$ <\\chi(x),\\chi(z)>$.
...
- 意味着 $ \\alpha\_i^\*和(y\_i(w^Tx\_i + b) - 1 + \\xi\_i^{\*})$中至少一个等于0
- 我们将后边的写成如下形式
    - $ (y\_i(w^\*\\cdot\\phi(x\_i)+b+\\xi\_i^\*))$
...
由于 $ \\frac{\\partial L}{\\partial w} = 0 \\;\\Rightarrow w = \\sum\\limits\_{i=1}^{m}\\alpha\_iy\_ix\_i$

$ w^\*=\\sum\\limits\_{j=1}^{m}a\_j^\*y\_i\\phi(x\_j)$

g(x)中消去w

$ g(x)=w^\*\\cdot\\phi(x)+b=\\sum\\limits\_{j=1}^{m}a\_j^\*y\_j\\phi(x\_j)\\cdot\\phi(x)+b$

## SMO算法基本思想

SMO算法采用了一种启发式算法，他每次只优化2个变量，将其他的变量视为常数，由于$ \\sum\\limits\_{i=1}^{m}a\_iy\_i=0$，假设我们将 $ a\_3,a\_4,\\cdots,a\_m$固定，那么 $ a\_1,a\_2$之间的关系也确定了，这样SMO将一个复杂的优化算法转化成一个比较简单的两变量优化问题。

为了以后表示方便我们定义 $ K\_{ij}=\\phi(x\_i)\\phi(x\_j)$

由于 $ a\_3,a\_4,\\cdots,a\_m$是常量，常量都从我们的目标函数去除，
...
- $ a\_1y\_1+a\_2y\_2=\\varsigma ; 0\\le a\_i\\le C;i=1,2$
- 由于$ y\_1,y\_2$只能取值1或者-1
- 综上所述，方程无非是
    - $ a\_1+a\_2=\\varsigma$
    - $ a\_1-a\_2=\\varsigma$
    - 这样$ a\_1,a\_2$都在同一个线段上，斜率为1或者-1（求偏导）。
    - 这样 $ a\_1,a\_2$在\[0,C\]和\[C,0\]形成的盒子里边，并且两者之间的关系直线的斜率只能是1或者-1.也就是i说$ a\_1,a\_2$的关系直线平行于\[0,C\]和\[C,0\]形成的盒子的对角线。

[![no img]](http://127.0.0.1/wp-content/uploads/2018/03/1042406-20161128221540099-1580490663.png)

因为 $ \\alpha\_1,\\alpha\_2$在一条线段上，所以2个变量的优化问题实际仅仅是一条变量的优化问题，这里我们不放最终假设是$ \\alpha\_2$的优化问题，由于我们采取的是启发式的迭代法，

- 假设我们上一轮迭代得到的解是 $ \\alpha\_1^{old},\\alpha\_2^{old}$，
- 假设沿着约束方向 $ \\alpha\_2$未经剪辑的解是 $ \\alpha\_2^{new,unc}$
- 本轮迭代完成的解是 $ \\alpha\_1^{new},\\alpha\_2^{new}$

由于$ \\alpha\_2^{new}$必须满足上图中的线段约束，假设L和H分别是上图中$ \\alpha\_2^{new}$所在线段的边界，那么我们很显然有
...
- 对左边图而言，$ y\_1 \\ne y\_2 \\Rightarrow \\alpha\_1-\\alpha\_2$
    - $ L=max(0,a\_2^{old}-a\_1^{old})$
    - $ H=min(C,C+a\_2^{old}-a\_1^{old})$
- 对有边图而言，$ y\_1 = y\_2 \\Rightarrow \\alpha\_1+\\alpha\_2$
    - $ L=max(0,a\_2^{old}+a\_1^{old}-C)$
    - $ H=min(C,a\_2^{old}+a\_1^{old})$

如上的其实很好理解。

- 我们这里假设$ y\_{1}\\ne y\_{2}$
    - 那么就有 $ \\alpha\_{1}^{old}-\\alpha\_{2}^{old}=\\alpha\_{1}^{new}-\\alpha\_{2}^{new}$
    - 然后继续 $ a\_{2}^{new}=\\alpha\_{1}^{new}+(\\alpha\_{2}^{old}-\\alpha\_{1}^{old})$
    - $ \\alpha\_1和\\alpha\_2$都要>=0,<=C
    - 将$ \\alpha\_{1}^{new}$分别代入0和C
    - $ \\alpha\_{2}^{old}-\\alpha\_{1}^{old}\\le\\alpha\_2^{new}\\le C+(\\alpha\_{2}^{old}-\\alpha\_{1}^{old})$

也就是说，假设我们通过求导得到 $ \\alpha\_2^{new,unc}$，则最终的$ a\_2^{new}$应该为
...
现在我们求 $ \\alpha\_2^{new,unc}$，将目标函数对$ \\alpha\_2$求偏导。

先写出目标函数。

$ W(\\alpha\_1,\\alpha\_2)=\\underbrace{ min }\_{\\alpha\_1,\\alpha\_2} \\frac{1}{2}a\_1^2K\_{11}+\\frac{1}{2}a\_2^2k\_{22}+a\_1a\_2y\_1y\_2k\_{12}+\\frac{1}{2}a\_1y\_1\\sum\\limits\_{i=3}^{m}a\_iy\_ik\_{1i}+\\frac{1}{2}a\_2y\_2\\sum\\limits\_{i=3}^{m}a\_iy\_ik\_{2i}-(\\alpha\_1+\\alpha\_2)$
...
- $ g(x)=w^\*\\cdot\\phi(x)+b=\\sum\\limits\_{j=1}^{m}\\alpha\_j^\*y\_iK(x,x\_j)+b^\*$
...
$ W(\\alpha\_1,\\alpha\_2)=\\underbrace{ min }\_{\\alpha\_1,\\alpha\_2} \\frac{1}{2}a\_1^2K\_{11}+\\frac{1}{2}a\_2^2k\_{22}+a\_1a\_2y\_1y\_2k\_{12}+a\_1y\_1v\_1+a\_2y\_2v\_2-(\\alpha\_1+\\alpha\_2)$

由于约束条件是$ \\alpha\_1y\_1+\\alpha\_2y\_2=\\varsigma，并且 y\_i^2=1$，可以得到 $ \\alpha\_1用\\alpha\_2$表达的式子
...
- 求 $ w,b,\\xi$最小值，对这三个变量求偏导 ，得到 \\begin{align}\\frac{\\partial L}{\\partial w} = 0 \\;\\Rightarrow w = \\sum\\limits\_{i=1}^{m}\\alpha\_iy\_ix\_i\\\\\\frac{\\partial L}{\\partial b} = 0 \\;\\Rightarrow \\sum\\limits\_{i=1}^{m}\\alpha\_iy\_i = 0\\\\\\frac{\\partial L}{\\partial \\xi} = 0 \\;\\Rightarrow C- \\alpha\_i – \\mu\_i = 0\\end{align}
...
- 完整的优化目标函数2，\\begin{align}\\underbrace{ min }\_{\\alpha} \\frac{1}{2}\\sum\\limits\_{i=1,j=1}^{m}\\alpha\_i\\alpha\_jy\_iy\_jx\_i^Tx\_j – \\sum\\limits\_{i=1}^{m}\\alpha\_i\\\\s.t. \\; \\sum\\limits\_{i=1}^{m}\\alpha\_iy\_i = 0\\\\0 \\leq \\alpha\_i \\leq C\\end{align}
    - 参考 $ L(w,b,\\xi,a,\\mu)=\\frac{1}{2}||w||\_2^2+C\\sum\\limits\_{i=1}^{m}\\xi\_i-\\sum\\limits\_{i=1}^{m}a\_i\\left\[y\_i(w^Tx\_i+b)-1+\\xi\_i\\right\]-\\sum\\limits\_{i=1}^{m}\\mu\_i\\xi\_i$
    - 这里的$ \\alpha\_{i}$一开始是定义 这个距离的约束系数。
    - $ \\alpha\_{i}^{\*}(y\_i(w^Tx\_i + b) – 1 + \\xi\_i^{\*}) = 0$
    - \\begin{array}{|c|c|c|c|}\\hline 已知&\\alpha\_i^\*=0&0<\\alpha\_i^\*<C&a\_i^\*=C\\\\\\hline \\frac{\\partial L}{\\partial \\xi}=C-\\alpha\_i-\\mu\_i=0&\\mu\_i=C\\ge 0&\\mu>0&\\mu\_i=0\\\\\\hline L(w,b,\\xi,\\alpha,\\mu)=\\cdots-\\sum\\limits\_{i=1}^m\\mu\_i\\xi\_i;中\\mu\_i\\xi\_i=0&\\xi\_i\\ge 0&\\xi=0&\\xi\_i\\ge 0\\\\\\hline\\alpha\_{i}^{\*}(y\_i(w^\*\\cdot\\phi(x\_i)+b+\\xi\_i^\*) = 0;&y\_i((w^\*\\cdot\\phi(x\_i)+b)\\ge 1&y\_i(w^\*\\cdot\\phi(x\_i)+b)=1&y\_i(w^\*\\cdot\\phi(x\_i)+b)\\le1\\\\\\hline\\\\设g(x)=w^\*\\cdot\\phi(x)+b&y\_ig(x\_i)\\ge 1&y\_ig(x\_i)=1&y\_ig(x\_i)\\le 1\\\\\\hline\\\\说明&点是支持向量或被正确分类&点在支持向量上&异常的点\\\\\\hline\\end{array}
- 设 $ g(x)=w^\*\\cdot\\phi(x)+b=\\sum\\limits\_{j=1}^{m}a\_j^\*y\_j\\phi(x\_j)\\cdot\\phi(x)+b$
- SMO算法基本思想是先优化2个变量，设其他的为常量，我们先设置 $ \\alpha\_1,\\alpha\_2$为变量，那么更新优化函数 $$\\underbrace{ min }\_{\\alpha\_{1},\\alpha\_{2}} \\frac{1}{2}\\alpha\_{1}^2K\_{11}+\\frac{1}{2}\\alpha\_{2}^2k\_{22}+\\alpha\_{1}\\alpha\_{2}y\_{1}y\_{2}k\_{12}+\\alpha\_{1}y\_{1}\\sum\\limits\_{i=3}^{m}a\_{i}y\_{i}k\_{1i}+\\alpha\_{2}y\_{2}\\sum\\limits\_{i=3}^{m}a\_{i}y\_{i}k\_{2i})-(\\alpha\_{1}+\\alpha\_{2})\\\\s.t. \\alpha\_1y\_1+\\alpha\_2y\_2=-\\sum\\limits\_{i=3}^{m}y\_i\\alpha\_i=\\varsigma\\\\0\\le \\alpha\_i\\le C;i=1,2$$
- 假设我们通过求导得到 $ \\alpha\_2^{new,unc}$，则最终的$ a\_2^{new}$应该为$$\\alpha\_2^{new}\\left\\{\\begin{align}&H&\\alpha\_2^{new,unc}>H\\\\&\\alpha\_2^{new,unc}&L\\le\\alpha\_2^{new,unc}\\le H\\\\&L&\\alpha\_2^{new,unc}<L\\end{align}\\right. $$
- 设置一些变量
    - 设$ E\_i=g(x\_i)-y\_i=\\sum\\limits\_{j=1}^{m}\\alpha\_jy\_jK(x,x\_j)+b-y\_i$
    - 设$ v\_i=\\sum\\limits\_{j=3}^{m}y\_{j}\\alpha\_{j}K(x\_{i},x\_{j})=g(x\_i)-\\sum\\limits\_{j=1}^{2}y\_j\\alpha\_{j}K(x\_i,x\_j)-b$
- $ \\alpha\_1=y\_1(\\varsigma-\\alpha\_2y\_2)$
- 得到只含2个变量的函数 $ W(\\alpha\_{1},\\alpha\_{2})=\\frac{1}{2}(\\varsigma-\\alpha\_{2}y\_{2})^{2}K\_{11}+\\frac{1}{2}\\alpha\_{2}^{2}K\_{22}+\\varsigma\\alpha\_{2}y\_{2}K\_{12}-\\alpha\_{2}^{2}K\_{12}+(\\varsigma v\_{1}-\\alpha\_{2}y\_{2}v\_{1})+\\alpha\_{2}y\_{2}v\_{2}-(y\_{1}\\varsigma-y\_{1}\\alpha\_{2}y\_{2}-\\alpha\_{2})$
- 求导，$ \\frac{\\partial W}{\\partial \\alpha\_{2}}=K\_{11}\\alpha\_{2}+K\_{22}\\alpha\_{2}-2K\_{12}\\alpha\_{2}-\\varsigma K\_{11}y\_{2}+\\varsigma y\_{2}K\_{12}-y\_{2}v\_{1}+y\_{2}v\_{2}+y\_{1}y\_{2}-1$
- 设导数=0 得到，$$(K\_{11}+K\_{22}-2K\_{12})\\alpha\_{2}=\\alpha\_{2}(K\_{11}-2K\_{12}+K\_{22})+y\_{2}(E\_{1}-E\_{2})$$
- $ \\alpha\_{2}^{new unc}=\\alpha\_{2}^{old}+\\frac{y\_{2}(E\_{1}-E\_{2})}{K\_{11}-2K\_{12}+K\_{22}}$
...
一般来说，我们首先选择违反 $ 0\\le \\alpha\_{i}^{\*}\\le C\\Rightarrow y\_{i}g(x\_{i})=1$,如果这些支持向量都满足KKT条件，再选择违反其他2个条件的点。

### 第二个变量的选择

SMO算法称选择第二个变量为内层循环，假设我们已经在外层循环中找到了$ \\alpha\_{1}$,第二个变量 $ \\alpha\_{2}$的选择标准是 $ |E1-E2|$有足够大的变化，

如果内层循环找到的点不能让目标函数有足够的下降，可以采用遍历向量点来做 $ \\alpha\_{2}$，直到目标函数有足够的下降，所有的支持向量做 $ \\alpha\_{2}$都不能让目标函数有足够的下降，可以跳出循环，重新选择 $ \\alpha\_{1}$

### 计算阀值b和差值$ E\_{i}$

在每次完成2个遍历的优化知后，需要重新计算阀值b，当 $ 0 \\le \\alpha\_{1}^{new}\\le C$时，我们有

$$ y\_{1}-\\sum\\limits\_{i=1}^{m}\\alpha\_{i}y\_{i}K\_{i1}-b\_{1}=0$$

- 我觉得这个是由 $ y\_{1}g(x\_{1})=1$的来的。

于是新的$ b\_{1}^{new}=y\_{1}-\\sum\\limits\_{i=3}^{m}\\alpha\_{i}y\_{i}K\_{i1}-\\alpha\_{1}^{new}y\_{1}K\_{11}-\\alpha\_{2}^{new}y\_{2}K\_{21}$

计算出$ E\_{1}$

$$E\_{1}=g(x\_{1})-y\_1== \\sum\\limits\_{i=3}^{m}\\alpha\_iy\_iK\_{i1} + \\alpha\_{1}^{old}y\_1K\_{11} + \\alpha\_{2}^{old}y\_2K\_{21} + b^{old} -y\_1$$

两边都有$ y\_1 - \\sum\\limits\_{i=3}^{m}\\alpha\_iy\_iK\_{i1}$，因此

$$b\_1^{new} = -E\_1 -y\_1K\_{11}(\\alpha\_{1}^{new} - \\alpha\_{1}^{old}) -y\_2K\_{21}(\\alpha\_{2}^{new} - \\alpha\_{2}^{old}) + b^{old}$$

同理 $ 0 \\leq \\alpha\_{2}^{new} \\leq C$，那么
...
- 几个简写
    - $ E\_i=g(x\_i)-y\_i=\\sum\\limits\_{j=1}^{m}\\alpha\_jy\_jK(x,x\_j)+b-y\_i$
    - $ K\_{ij}=\\phi(x\_i)\\phi(x\_j)$
    - 这个区间 ： $ L\\le \\alpha\_2^{new}\\le H $ ,
        - ![no img]
        - 对左边图而言，$ y\_1 \\ne y\_2 \\Rightarrow \\alpha\_1-\\alpha\_2$
            - $ L=max(0,a\_2^{old}-a\_1^{old})$
            - $ H=min(C,C+a\_2^{old}-a\_1^{old})$
...
- 算法：
    ...
    6. 求 $ \\alpha\_{1}^{k+1}$
        - $ \\alpha\_{1}^{k}y\_{1}+\\alpha\_{2}^{k}y\_{2}=\\alpha\_{1}^{k+1}y\_{1}+\\alpha\_{2}^{k+1}y\_{2}$
    7. 求 $ b^{k+1}$
        - $ b\_1^{new} = -E\_1 -y\_1K\_{11}(\\alpha\_{1}^{new} – \\alpha\_{1}^{old}) -y\_2K\_{21}(\\alpha\_{2}^{new} – \\alpha\_{2}^{old}) + b^{old}$
        - $ b\_2^{new} = -E\_2 -y\_1K\_{12}(\\alpha\_{1}^{new} – \\alpha\_{1}^{old}) -y\_2K\_{22}(\\alpha\_{2}^{new} – \\alpha\_{2}^{old}) + b^{old}$
        - $ b^{new} = \\frac{b\_1^{new} + b\_2^{new}}{2}$
    ...
    10. 如果条件满足，返回$ \\alpha^{k+1}$，否则进入步骤2.
...
- 在SVM分类模型中，
    - 在硬间隔中
        - 我们的目标函数是让 $ \\frac{1}{2}||w||\_{2}^{2}$最小。
        - 同时让各个训练集中的点尽量远离自己类别一遍的支持向量
            - $ y\_{i}(w\\cdot\\phi(x\_{i})+b)\\ge 1$
    - 如果是软间隔，就加入一个松弛变量，$ \\xi\_{i}\\ge 0$
        - 目标函数是 $ \\frac{1}{2}||w||\_{2}^{2} +C\\sum\\limits\_{i=1}^{m}\\xi\_{i}$
        - 对应的约束条件变成
            - $ y\_{i}(w\\cdot\\phi(x\_{i})+b)\\ge 1-\\xi\_{i}$

回归模型中没有类别，而是对于每个点 $ (x\_{i},y\_{i})$，尽量拟合到一个线性模型 $ y\_{i}=w\\cdot\\phi(x\_{i})+b$

SVM线性回归的损失函数是先定义一个常量 $ \\epsilon >0$ ，对于 $ (x\_{i},y\_{i})$

- 如果 $ |y\_{i}-w\\cdot\\phi(x\_{i})-b|\\le\\epsilon $
    - 则认为没有损失，这不就是软间隔中的边界内吗？
        - 和均方差不同，均方差在这个边界内也是算进损失的。
- 如果 $ |y\_{i}-w\\cdot\\phi(x\_{i})-b|>\\epsilon $
    - 则损失为 $ |y\_{i}-w\\cdot\\phi(x\_{i})-b|-\\epsilon $
...
类似软间隔中加入松弛变量。但由于是绝对值，实际上2个不等式，也就是2边都需要松弛变量（我认为可以理解成上下边界），我们定义2个松弛变量。$ \\xi\_i^{\\lor}, \\xi\_i^{\\land}$，则我们的SVM回归模型在加入松弛变量后变为
...
其中 $ \\mu^{\\lor} \\geq 0, \\mu^{\\land} \\geq 0, \\alpha\_i^{\\lor} \\geq 0, \\alpha\_i^{\\land} \\geq 0$均为拉格朗日系数。
...
在SVM分类模型中，KKT条件的对偶互补条件为 $ \\alpha\_{i}^{\*}(y\_i(w \\bullet \\phi(x\_i) + b) - 1+\\xi\_i^{\*}) = 0$，而在回归模型中，对偶互补条件类似如下
