...
$ y\_t = softmax(h\_t w\_{hy} + b\_y) $

$ h\_t = tanh(x\_t w\_{xh} + h\_{t-1}w\_{hh}) $

- 输入特征 $ x$
- 当前记忆体输出特征 $ y\_t $
- 当前记忆体存储状态信息 $ h\_t$
- 上一时刻记忆体存储状态信息 $ h\_{t-1}$
- 参数矩阵 $ w\_{xh}, w\_{hh}, w\_{hy}$
- 偏执项 $ b\_h $
- 前向传播时：记忆体内存储的状态信息$ h\_t$，在每个时刻都被刷新，三个参数矩阵$ w\_{xh} , w\_{hh}, w\_{hy} $ 自始至终都是固定不变的。
- 反向传播时，$ w\_{xh} , w\_{hh}, w\_{hy} $ 被梯度下降法更新
